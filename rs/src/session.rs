...
//! For reconnecting agents, `CliSession::new()` accepts raw channels (used by external backends).
...
impl CliSession {
    /// Create a `CliSession` from raw channels (for external process backends).
    ...
    /// For remote-backend sessions this is a no-op (returns `false`).
