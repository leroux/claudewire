...
claudewire works with these types -- it never imports from any
specific transport backend. The host application's wiring layer provides
an adapter from a concrete transport to this protocol.
"""
...
class ProcessEventQueue(Protocol):
    ...
    (like a host application's translating queue) satisfy this protocol.
    """
...
class ProcessConnection(Protocol):
    ...
    Any backend (remote process multiplexer, direct subprocess, SSH, etc.) can implement
