...
2. **Runtime log analysis** — Running the CLI with `--output-format stream-json` (via `-p` for single-turn or the SDK's `SubprocessCLITransport`) produces real NDJSON protocol traffic on stdout. The bridge-stdio logger captures this in production; `stdio-spy` (sibling package) can capture ad-hoc sessions.
...
grep '<<< STDOUT' ~/logs/bridge-stdio.log | grep -oP '"type":"[^"]+"' | sort | uniq -c | sort -rn
```
